...
Class Support extends MY_Controller {
	...
	public function addRequeststaffmsg(){
		...
		$response2 = $this->Manage_Support_Model->sendTicketMessage($_REQUEST['remarks'], $reqdata->ticketno, $reqdata->emailid);
	...
	public function changeticketstatus($statusid, $id) {
		...
		if($reqdata->id > 0){
			...
			switch ($statusid) {
				case '1':
					$response = $this->Manage_Support_Model->sendTicketOpenMessage($reqdata->ticketno, $reqdata->emailid, $reqdata->mobile);
					break;

				case '2':
					$response = $this->Manage_Support_Model->sendTicketProcessMessage($reqdata->ticketno, $reqdata->emailid, $reqdata->mobile);
					break;

				case '3':
					$response = $this->Manage_Support_Model->sendTicketClosedMessage($reqdata->ticketno, $reqdata->emailid, $reqdata->mobile);
					break;

				case '4':
					$response = $this->Manage_Support_Model->sendTicketResolvedMessage($reqdata->ticketno, $reqdata->emailid, $reqdata->mobile);
					break;
