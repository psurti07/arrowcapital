...
Class Sms extends MY_Controller {
	...
	public function sendTestsms(){
		$smsmessage = $dataset = $smsresponse = "";
		...
		if($_REQUEST['mobile'] != '' && $smsmessage != "") {
			...
			if($_REQUEST['smstype'] == 1) {
			    $eligibilityamt = "5,00,000";
				$smsmessage = str_replace("<#cronamount>",$eligibilityamt,$smsmessage);
				
				$dataset = "<sms><user>".SMS_OBB_USERNAME."</user><password>".SMS_OBB_API_KEY."</password><mobiles>".$_REQUEST['mobile']."</mobiles><message>".$smsmessage."</message><accusage>1</accusage><senderid>".$smssendid."</senderid></sms>";

				$smsresponse = sendxmlSMSobb($dataset);
			}
			else if($_REQUEST['smstype'] == 2) {
			    $eligibilityamt = "5,00,000";
	...
	public function remarketinglog(){
		...
		$this->load->model('Manage_Sms_Model');
		$loglist = $this->Manage_Sms_Model->getremarketinglog($dt_to, $dt_from, $parentid);

		$this->load->view('sms-log-list',['loglist'=>$loglist, 'dt_to'=>$dt_to, 'dt_from'=>$dt_from, 'parentid' => $parentid]);
	}
	...
	public function logdetails($id){
		...
		$this->load->view('sms-log-details',['logdetails'=>$logdetails]);
	}
