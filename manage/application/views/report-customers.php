...
<script type="text/javascript">
	window.onload = function() {
		document.getElementById("122").className += " active";
	}
</script>
...
<div class="content-header row">
	<div class="content-header-left col-md-12 col-12 mb-1">
		<h1 class="content-header-title text-uppercase">Report - Customer Registrations</h1>
	</div>
</div>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$('.model_data').on('click', function() {
			...
			if (month) {
				$.ajax({
					url: '<?php echo base_url("report/customersdaywise"); ?>',
					type: "POST",
					data: {
						'month': month,
						'year': year
					...
					success: function(response) {
						...
						for (i = 0; i < response.length; i++) {
							...
							raw_data += '<td class="text-right">' + response[i].totaluser + '</td>';
							raw_data += '</tr>';
							total_cust += parseInt(response[i].totaluser);
						}
						if (raw_data) {
							$('#subtotalcust').html(total_cust);
						}
