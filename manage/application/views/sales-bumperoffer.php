...
<script type="text/javascript">
	window.onload = function () {
		...
		document.getElementById("1522").className += " active";
	}
</script>

<div class="content-header row">
	<div class="content-header-left col-md-6 col-12 mb-1">
		<h1 class="content-header-title text-uppercase">Bumper Offer Sales</h1>
	</div>
	<div class="content-header-right col-md-6 col-12 mb-1">
		<?php echo form_open('offer/bumperoffer', array('id' => 'filterForm', 'class' => 'form-horizontal', 'novalidate' => 'novalidate')); ?>
								...
								<tbody>
									...
									if (count($saleslist)) {
										...
										foreach ($saleslist as $row) {
											...
											if ($row->isCustomer == 1) {
												echo "<td class='text-center'>" . anchor("offer/offerstatus/6/{$row->isCustomer}/{$row->id}", 'Customer', 'class="btn btn-icon btn-outline-primary btn-sm"') . "</td>";
											} else {
												echo "<td class='text-center'>" . anchor("offer/offerstatus/6/{$row->isCustomer}/{$row->id}", 'Lead', 'class="btn btn-icon btn-outline-success btn-sm"') . "</td>";
											}
