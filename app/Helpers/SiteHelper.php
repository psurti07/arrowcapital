        ...
        foreach ($banks as $bank) {
            $data['list'] .= '<div class="col company" data-index="' . $loop . '">
                            <a href="javascript:;" class="in_tool it-1 r-10">
                                <div class="bg--white-100 block-shadow r-10 mb-0 px-lg-3 py-lg-2 p-2">
                                    <img class="img-fluid w-100" src="https://manage.arrowcapital.in/public/upload/banks/'.$bank->bank_image.'" alt="' . $bank->bank_name . '">
                                </div>
                                <h6 class="s-14 w-700">' . $bank->bank_name . '</h6>
                            </a>
                        </div>';

            $data['carousel'] .= '<div class="bg--white-100 border border-gray r-10 mb-0 px-lg-3 py-lg-2 p-2">
                                    <a href="javascript:;">
                                        <img class="img-fluid w-100" src="https://manage.arrowcapital.in/public/upload/banks/'.$bank->bank_image.'" alt="' . $bank->bank_name . '">
                                    </a>
                                </div>';
                ...
                <li class="accordion-item mb-10">
                    ...
                    <div class="accordion-panel">
                        <div class="accordion-panel-item">
                            <div class="faqs-2-answer">
                                <p>
                                   We request you to kindly call the company on +91-73835 26761 between 10 AM to 5 PM- Monday to Saturday (only business days). Allow us to discuss your concerns, and we will ensure that you receive the best solutions possible. 
                                </p>
                            </div>
                        </div>
                    </div>
...
if(!function_exists('sendBrevoHtmlMail')){
    function sendBrevoHtmlMail($maildata, $subject = '', $message = '', $sendmail = '', $attachmentPath = ''){
        ...
        $data["sender"]["email"] = 'info@arrowcapital.in';
...
if(!function_exists('sendPaymentGreetings')){
    function sendPaymentGreetings($fullname, $mobile, $email){
        /*if($mobile != '') {
            $smsmessage = "Dear Customer, Congratulations! Your loan application has been successfully submitted. Our Customer Executive will call you shortly. Thanks, Arrow Capital";
            $smsresponse = sendtextSMSobb($mobile, $smsmessage, 'main');
        }*/
        if($email != '') {
            // Send email
            $subject = "Welcome to Arrow Capital";
