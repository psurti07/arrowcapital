...
class Data_Test_And_Clean_Model extends CI_Model
{
	...
	public function delete_duplicate_user_carddata()
	{
		
		$sql_invoice = "DELETE s1 FROM invoice s1, invoice s2 WHERE s1.cardid < s2.cardid AND s1.userid = s2.userid";
		$sql_subscription = "DELETE s1 FROM subscription_order s1, subscription_order s2 WHERE s1.id < s2.id AND s1.userid = s2.userid";
		$query_invoice = $this->db->query($sql_invoice);
		$affectedRows_inv = $this->db->affected_rows();
		$query_subscription = $this->db->query($sql_subscription);
		...
		if ($affectedRows_mem) {
			$str .= "subscription table total (" . $affectedRows_mem . ") records affected. <br/>";
		} else {
			$str .= "subscription table (0) records affected. <br/>";
		}
