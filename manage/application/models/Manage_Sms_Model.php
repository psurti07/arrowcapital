...
Class Manage_Sms_Model extends CI_Model {
	...
	public function sendcustomsms($target=99, $message=''){
		...
		if($message != ''){
			...
			if($target != 99) {
				...
				if(count($userlist) > 0) {
					...
					foreach($arrpart as $res) {
						...
						foreach($res as $row) {
							$dataset .= "<sms><user>".SMS_OBB_USERNAME."</user><password>".SMS_OBB_API_KEY."</password><mobiles>".$row->mobile."</mobiles><message>".$message."</message><accusage>1</accusage><senderid>".$smssendid."</senderid></sms>";
			...
			$dataset .= "<sms><user>".SMS_OBB_USERNAME."</user><password>".SMS_OBB_API_KEY."</password><mobiles>9023987358</mobiles><message>".$message."</message><accusage>1</accusage><senderid>".$smssendid."</senderid></sms>";
