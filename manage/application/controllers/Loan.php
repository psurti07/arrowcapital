...
Class Loan extends MY_Controller {
	...
	public function offline($loan=''){
		...
		$this->load->model('Manage_Loan_Enquiry_Model');
		$enquirylist = $this->Manage_Loan_Enquiry_Model->getenquiryloan($loan, $dt_to, $dt_from);
		$this->load->view('loanenquiry-individual',['loan'=>$loan, 'enquirylist'=>$enquirylist, 'dt_to'=>$dt_to, 'dt_from'=>$dt_from]);
	}
	...
	public function addAppstatus(){
		...
		if(isset($_FILES['sanctionletter']) && $_FILES['sanctionletter']['name'] != '') {
			$this->load->model('Manage_General_Model');
			$sanction_letter = $this->Manage_General_Model->single_file_upload('sanctionletter', 'img/sanctionletter', 'pdf', 0);
		}
		...
		$response2 = $this->Manage_Loan_Enquiry_Model->sendStatusMessage($_REQUEST['remarks'], $userdata->mobile, $userdata->email); 

		redirect('loan/appdetails/'.$_REQUEST['applicationid']);
	}

	public function downloadfile($id, $sanctionletter) {
		$this->load->model('Manage_General_Model');
