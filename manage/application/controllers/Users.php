...
Class Users extends MY_Controller {
		...
		if($isuser == 0) {
			...
			if($userid > 0) {
				...
				$data4 = array(
					...
					'expiry_date' => date('Y-m-d', strtotime('+6 months', strtotime($regdate))),
					'card_number' => $cardno,
					'amount' => $grandtotal,
					'paymentid' => $paymentid,
					'isActive' => 1,
				...
				$subscriptionid = $this->Manage_User_Model->subscriptionorder($data4);

				$this->load->model('Manage_Site_Model');
				$invoiceno = $this->Manage_Site_Model->getinvoiceno();
				...
				$response4 = $this->Manage_User_Model->updatepayoutdata($userid, $data4);
				...
				$sent = $this->Manage_User_Model->sendSuccessGreetings($maildata);
	...
	public function kycunverifieddocs(){
		$this->load->model('Manage_User_Model');
		$userlist = $this->Manage_User_Model->getkycdocumentlist(0);
		$this->load->view('users-kyc-unverified-docs',['userlist'=>$userlist]);
	}

	public function payoutunverifieddocs(){
		$this->load->model('Manage_User_Model');
		$userlist = $this->Manage_User_Model->getpayoutdocumentlist(0);
		$this->load->view('users-payout-unverified-docs',['userlist'=>$userlist]);
	}
	...
	public function payoutdocuments($id){
		$payoutstatus = 0;
		$documentlist = array();
		...
		$isdata = $this->Manage_User_Model->checkpayoutdocuments($datares->id);
		if($isdata > 0) {
			$payoutstatus = $this->Manage_User_Model->getpayoutdocstatus($id);
			$documentlist = $this->Manage_User_Model->getpayoutdocuments($datares->id);
		}
		...
		$this->load->view('user-payout-documents',['userdata'=>$userdata, 'payoutstatus'=>$payoutstatus, 'documentlist'=>$documentlist]);
	}

	public function downloadpayoutdoc($id = NULL, $document = NULL) {
		if($id != NULL && $document != NULL) {
			$this->load->model('Manage_General_Model');
			$doc = $this->Manage_General_Model->filedownload('kycdocuments', $document);
			redirect('users/payoutdocuments/'.$id);
		}
		else {
			redirect('users');
		}
	...
	public function reuploadpayoutdoc($document = NULL, $id = NULL) {
		if($id != NULL && $document != NULL) {
			$data = array(
				'rec_date' => date('Y-m-d H:i:s'),
				'isVerified' => 0,
			...
			$this->load->model('Manage_User_Model');
			$doc = $this->Manage_User_Model->reuploadpayoutdocument($id, $data);
			redirect('users/payoutdocuments/'.$id);
		}
		else {
			redirect('users');
	...
	public function payoutdocverification($status, $id) {
		$this->load->model('Manage_User_Model');
		$response = $this->Manage_User_Model->verifypayoutdocuments($id, $status);

		if($status == 1) {
			$userdata = $this->Manage_User_Model->getuserdata($id);
			$response3 = $this->Manage_User_Model->sendpayoutverifymessage($userdata->mobile, $userdata->email);
		}
	...
	public function subscriptionplan($id){
		...
		$this->load->view('user-subscription-plan',['userdata'=>$userdata, 'carddetails'=>$carddetails]);
	}
	...
	public function downloadinvoice($id, $cardid){
		...
		if(!empty($invdetails['invoiceinfo'])) {
	        $html = $this->load->view('user-invoice', ['invdetails'=>$invdetails], true);
		}
		else {
			$html = $this->load->view('user-invoice-old', ['invdetails'=>$invdetails], true);
		}

		$this->pdf->createPDF($html, $invoiceno, false);

		/* redirect('users/subscriptionplan/'.$id); */
	}
	...
	public function referrallist($id){
		...
		$this->load->model('Manage_User_Model');
		$datares = $this->Manage_User_Model->getuserdata($id);
		$reflist = $this->Manage_User_Model->getreferallist($dt_to, $dt_from, $id);
		...
		$this->load->view('user-referral-list',['userdata'=>$userdata, 'reflist'=>$reflist, 'dt_to'=>$dt_to, 'dt_from'=>$dt_from]);
	}
	...
	public function subscriptionlist($cardtype = '11'){
		$dt_to = date('Y-m-d', strtotime('-1 days'));
		$dt_from = date('Y-m-d');
		...
		$this->load->model('Manage_User_Model');
		$subscriptionlist = $this->Manage_User_Model->getsubscriptionlist($dt_to, $dt_from, $cardtype);

		$this->load->view('subscription-list',['subscriptionlist'=>$subscriptionlist, 'dt_to'=>$dt_to, 'dt_from'=>$dt_from, 'cardtype'=>$cardtype]);
	}
	...
	public function converttocustomer(){
		...
		if(isset($_REQUEST['userid'])) {
			...
			if($subscriptionid != 0){
				...
				$response4 = $this->Manage_User_Model->updatepayoutdata($userdata->id, $data4);
